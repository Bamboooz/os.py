...
def wmic_get_number_of_external_drives():
    ...
    """
    return len(wintools.parse_wmic('wmic logicaldisk where drivetype=2 get DeviceID, VolumeName, Description'))
...
def wmic_get_external_drives():
    ...
    using the WMIC command to query the system for logical disks with a drivetype of 2 (which corresponds to removable drives).
    """
    output = wintools.parse_wmic("wmic logicaldisk where drivetype=2 get DeviceID")
...
def powershell_get_number_of_external_drives():
    ...
    """
    devices = wintools.read_powershell_command('Get-WmiObject Win32_LogicalDisk | Where-Object {$_.DriveType -eq 2} | Select-Object -ExpandProperty DeviceID')
